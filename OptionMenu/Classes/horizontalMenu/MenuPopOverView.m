...
- (void)didTapLeftArrowButton:(UIButton *)sender {
    int width = [self currentScreenBoundsDependOnOrientation].size.width;
    float popoverMaxWidth = width - 2 * kPopOverViewPadding;
...
- (void)didTapRightArrowButton:(UIButton *)sender {
    int width = [self currentScreenBoundsDependOnOrientation].size.width;
//    NSLog(@"width = %i", [self currentScreenBoundsDependOnOrientation].size.width);
//    NSLog(@"screen width = %i", [UIScreen mainScreen].bounds.size.width);
    float popoverMaxWidth = width - 2 * kPopOverViewPadding;
